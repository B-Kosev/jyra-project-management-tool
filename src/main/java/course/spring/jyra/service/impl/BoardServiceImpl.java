...
public class BoardServiceImpl implements BoardService {
	...
	public Board findById(Integer id) {
		return boardRepository.findById(id)
				.orElseThrow(() -> new EntityNotFoundException(String.format("Board with ID=%s not found.", id)));
	}

	@Override
	public Board findByProjectId(Integer projectId) {
		return boardRepository.findAll().stream().filter(board -> board.getProject().getId().equals(projectId)).findFirst()
				.orElseThrow(() -> new EntityNotFoundException(String.format("Board with project ID=%s not found.", projectId)));
	}
