...
public class SprintResultServiceImpl implements SprintResultService {
	...
	public SprintResult findById(Integer id) {
		return sprintResultRepository.findById(id)
				.orElseThrow(() -> new EntityNotFoundException(String.format("Sprint result with ID=%s not found.", id)));
	}

	@Override
	public SprintResult create(SprintResult sprintResult, Integer sprintId) {
		Optional<SprintResult> result = sprintResultRepository.findAll().stream().filter(sr -> sr.getSprint().getId().equals(sprintId))
				.findFirst();
		if (result.isPresent()) {
			throw new ExistingEntityException(String.format("There is existing sprint result for sprint with ID=%s.", sprintId));
		}
...
//		}
	...
	public SprintResult findBySprintId(Integer id) {
		return sprintResultRepository.findAll().stream().filter(sprintResult -> sprintResult.getSprint().getId().equals(id)).findFirst()
				.orElseThrow(() -> new EntityNotFoundException(String.format("Sprint result for sprint with ID=%s not found.", id)));
	}
	...
	private int calculateTeamVelocity(SprintResult sprintResult) {
		return sprintResult.getSprint().getTasks().stream()
				...
				.mapToInt(TaskResult::getActualEffort).sum();
	}
